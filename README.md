# WaterPulse AI Agent V8.3 — 字段治理与界面布局修正版
...
## V8.3 字段治理更新
- `data/field_dictionary.csv`：纳入 E 组 51 个跨模块字段定义，供 Agent 做字段来源、必填性、时间/空间粒度与数据身份判断。
- `data/field_gap_register.csv`：纳入 E 组 10 个字段缺口，用于 Missing/Partial/Insufficient 路由和数据补充提示。
- 两表是数据治理合同，不覆盖已冻结的 D/C 模型语义；发现冲突时保留兼容性警告并以研究报告/确定性 Schema 为准。
- `/api/data-contract` 可供内部 QA 检查当前字段合同加载状态；前端不直接展示后台表格。
